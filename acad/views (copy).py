...
from .models import StudentRel,TeacherRel,HodRel,Student,Teacher,Marks,Subject,Batch,Attendance,CoverNews,TeacherNews,TeacherEvents,StudentEvents
...
def statshow(request,title):
	...
	if user.is_authenticated():
		...
		sub = Subject.objects.filter(batch=group).filter(subject='RDBMS')
		for s in students:			
			attend=Attendance.objects.filter(student=s).filter(subject=sub)
			atten1=len(attend)
			for t in attend:
				if t.p_a == True:
					i = i+1
			...
			if j<0.75:
				...
				if j<=0.85:
					...
					atten=atten+1 
			i=0			
		return render(request,'ucoe/Statistics.html',{'count':count,'count1':count1,'atten':atten,'list2':list2,'list3':list3,'list4':list4})
